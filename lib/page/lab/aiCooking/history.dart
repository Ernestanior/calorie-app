...
class AiCookingHistoryPage extends StatefulWidget {
  const AiCookingHistoryPage({super.key});

  @override
  State<AiCookingHistoryPage> createState() => _AiCookingHistoryPageState();
}

class _AiCookingHistoryPageState extends State<AiCookingHistoryPage> {
  ...
  Future<void> _loadHistory({bool loadMore = false}) async {
    ...
    try {
      final response = await yifanRecipeResponsePage(_currentPage, _pageSize,1);
  ...
  void _onItemTap(Map<String, dynamic> item) {
    ...
    if (responseDto == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('RECIPE_DATA_NOT_EXISTS'.tr)),
      );
      return;
    }

    // 构建食材列表
    final List<String> ingredientList = item['ingredientList'] is List
        ? (item['ingredientList'] as List).map((e) => e.toString()).toList()
        : [];
    
    // 构建 selectedIngredients（从 ingredientList 创建）
    final List<Map<String, dynamic>> selectedIngredients = ingredientList.map((name) {
      return {'name': name, 'displayName': name};
    }).toList();

    // 获取菜系信息
    final String cuisineName = item['cuisine']?.toString() ?? '';
    // 从 responseDto 中获取 cuisineId（如果有），否则使用 0
    int cuisineId = 0;
    if (responseDto['cuisine'] != null) {
      // 可以通过 cuisine 名称查找对应的 id，这里简化处理
    }

    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => RecipeDetailFromHistoryPage(
          recipeData: responseDto,
          ingredients: ingredientList,
          cuisineId: cuisineId,
          cuisineName: cuisineName,
          customPrompt: item['prompt']?.toString() ?? '',
  ...
  Widget _buildHistoryItem(Map<String, dynamic> item) {
    final Map<String, dynamic>? responseDto = item['responseDto'] as Map<String, dynamic>?;
    final String name = responseDto?['name']?.toString() ?? item['name']?.toString() ?? 'AI_COOKING_DEFAULT_RECIPE_NAME'.tr;
    final String cuisineRaw = responseDto?['cuisine']?.toString() ?? item['cuisine']?.toString() ?? '';
    final String cuisine = cuisineZhNameToKey.containsKey(cuisineRaw) 
        ? cuisineZhNameToKey[cuisineRaw]!.tr 
        : cuisineRaw;
                              ...
                              children: [
                                if (cuisine.isNotEmpty)
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: const Color.fromARGB(255, 120, 208, 125).withOpacity(0.1),
